...
let chalk;
try {
        chalk = require("./core/func/colors.js").colors;
}
catch (e) {
        chalk = require("chalk");
}
...
let pathLanguageFile = `${process.cwd()}/core/languages/${langCode}.lang`;
if (!fs.existsSync(pathLanguageFile)) {
        log.warn("LANGUAGE", `Can't find language file ${langCode}, using default language file "${path.normalize(`${process.cwd()}/core/languages/en.lang`)}"`);
        pathLanguageFile = `${process.cwd()}/core/languages/en.lang`;
}
...
(async () => {
        ...
        try {
                if (typeof packageHTML === 'object') {
                        packageJSON = JSON.stringify(packageHTML);
                } else {
                        // Attempt to parse if it's a GitHub HTML page (as seen in the original code's logic)
                        const splitReact = packageHTML.split('data-target="react-app.embeddedData">');
                        if (splitReact.length > 1) {
                                const json = splitReact[1].split('</script>')[0];
                                packageJSON = JSON.stringify(JSON.parse(json).payload.blob.rawLines.join('\n'));
                        } else {
                                // Assume it's raw JSON text
                                packageJSON = packageHTML;
                        }
                }
                JSON.parse(packageJSON); // Validate JSON
        } catch (e) {
                return log.error("ERROR", "Failed to parse package.json from GitHub");
        }
