...
module.exports = async function (databaseType, userModel, api, fakeGraphql) {
                ...
                const queue = new Promise(async function (resolve_, reject_) {
                        try {
                                ...
                                function normalizeUsername(userInfo) {
                                        if (userInfo?.vanity)
                                                return String(userInfo.vanity).toLowerCase();
                                ...
                                let userData = {
                                        ...
                                        username: normalizeUsername(userInfo), // ✅ FIX
                                        exp: 0,
                                        money: 0,
                                        diamond: 0, // 👈 next feature
                                        banned: {},
                                        settings: {},
                                        data: {},
                ...
                return new Promise(async function (resolve, reject) {
                        taskQueue.push(async function () {
                                ...
                                const newData = {
                                                ...
                                                username: normalizeUsername(updateInfoUser),
        ...
        async function get_(userID, path, defaultValue, query) {
                ...
                // Ensure diamond field exists with default value
                if (userData.diamond === undefined || userData.diamond === null)
                        userData = { ...userData, diamond: 0 };

                // Ensure username is set (fallback to vanity or generate from name)
                if (!userData.username) {
                        if (userData.vanity)
                                userData.username = String(userData.vanity).toLowerCase();
                        else if (userData.name)
                                userData.username = String(userData.name).toLowerCase().replace(/\s+/g, "").replace(/[^a-z0-9._]/g, "");
                        else
                ...
                taskQueue.push(async function () {
                        try {
                                if (isNaN(userID))
                                        throw new CustomError({
                                                name: "INVALID_USER_ID",
                                                message: `The first argument (userID) must be a number`
                                        });

                                const diamond = await get_(userID, "diamond");
                                resolve(diamond);
                        }
                        catch (err) {
                                reject(err);
        ...
        async function addDiamond(userID, diamond, query) {
        return new Promise((resolve, reject) => {
                taskQueue.push(async function () {
                        try {
                                if (isNaN(userID) || isNaN(diamond))
                                        throw new CustomError({
                                                name: "INVALID_ARGUMENT",
                                                message: "userID and diamond must be numbers"
                                        });

                                if (!global.db.allUserData.some(u => u.userID == userID))
                                        await create_(userID);

                                const current = await get_(userID, "diamond");
                                const newDiamond = current + diamond;

                                const userData = await save(userID, newDiamond, "update", "diamond");
        ...
        async function subtractDiamond(userID, diamond, query) {
        return new Promise((resolve, reject) => {
                taskQueue.push(async function () {
                        try {
                                if (isNaN(userID) || isNaN(diamond))
                                ...
                                const current = await get_(userID, "diamond");
                                const newDiamond = Math.max(0, current - diamond);
