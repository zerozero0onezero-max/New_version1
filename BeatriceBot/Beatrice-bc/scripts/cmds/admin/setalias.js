	...
	ncStart: async function ({ message, event, args, threadsData, globalData, role, getLang }) {
		...
		switch (args[0]) {
			case "add": {
				...
				if (!global.BeatriceBC.commands.has(commandName))
				...
				if (args[3] == '-g') {
					if (role > 1) {
						...
						if (global.BeatriceBC.aliases.has(alias))
							return message.reply(getLang("aliasExist", alias, global.BeatriceBC.aliases.get(alias)));
						...
						global.BeatriceBC.aliases.set(alias, commandName);
				...
				if (global.BeatriceBC.commands.get(alias))
			...
			case "rm": {
				...
				if (args[3] == '-g') {
					if (role > 1) {
						...
						await globalData.set('setalias', globalAliasesData, 'data');
						global.BeatriceBC.aliases.delete(alias);
