...
module.exports = {
        ...
        ncStart: async function ({ message, event, args, threadsData, getLang, usersData, api }) {
                ...
                const time = moment().tz(global.BeatriceBC.ncsetting.timeZone).format('HH:mm:ss DD/MM/YYYY');
                const data = {
                        id: target,
                        time,
                        ...
                        if (members.some(item => item.userID == target)) {
                                if (adminIDs.includes(api.getCurrentUserID())) {
                                        ...
                                        message.send(getLang('needAdmin'), (err, info) => {
                                                global.BeatriceBC.onEvent.push({
                                                        messageID: info.messageID,
                                                        ncStart: ({ event }) => {
                                                                if (event.logMessageType === "log:thread-admins" && event.logMessageData.ADMIN_EVENT == "add_admin") {
                                                                        const { TARGET_ID } = event.logMessageData;
                                                                        if (TARGET_ID == api.getCurrentUserID()) {
                                                                                api.removeUserFromGroup(target, event.threadID, () => global.BeatriceBC.onEvent = global.BeatriceBC.onEvent.filter(item => item.messageID != info.messageID));
                                                                        }
                                                                }
                                                        }
                                                });
                                                                ...
                                                                global.BeatriceBC.onEvent.push({
                                                                        ...
                                                                        ncStart: ({ event }) => {
                                                                                if (event.logMessageType === "log:thread-admins" && event.logMessageData.ADMIN_EVENT == "add_admin") {
                                                                                        const { TARGET_ID } = event.logMessageData;
                                                                                        if (TARGET_ID == api.getCurrentUserID()) {
                                                                                                api.removeUserFromGroup(userFbId, event.threadID, () => global.BeatriceBC.onEvent = global.BeatriceBC.onEvent.filter(item => item.messageID != info.messageID));
                                                                                        }
                                                                                }
                                                                        }
                                                                });
