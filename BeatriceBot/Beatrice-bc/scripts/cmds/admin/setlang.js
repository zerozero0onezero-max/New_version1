...
module.exports = {
        ...
        ncStart: async function ({ message, args, getLang, threadsData, role, event }) {
                ...
                if (["-g", "-global", "all"].includes(args[1]?.toLowerCase())) {
                        ...
                        global.BeatriceBC.ncsetting.language = langCode;
                ...
                return message.reply((global.BeatriceBC.commands.get("setlang")?.langs[langCode]?.setLangForCurrent || "Set default language for current chat: %1").replace("%1", langCode));
        }
};