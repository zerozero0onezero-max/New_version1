...
let patchrightError = '';
let environmentReport = checkEnvironment({ includeBrowser: true });
...
function attachPageBehavior(nextPage: any) {
  nextPage.on('popup', async (popup: any) => {
    await popup.waitForLoadState('domcontentloaded', { timeout: 10000 }).catch(() => undefined);
    const popupUrl = popup.url();
    if (/^https?:\/\//i.test(popupUrl)) {
      await nextPage.goto(popupUrl, { waitUntil: 'domcontentloaded', timeout: 45000 }).catch(() => undefined);
      lastUrl = nextPage.url();
    }
    await popup.close().catch(() => undefined);
  });
  nextPage.route('**/*', (route: any) => {
...
async function browserStats() {
  ...
  const pageMemory = page
    ? await page.evaluate(() => {
        const memory = (performance as Performance & {
          memory?: { usedJSHeapSize?: number };
        }).memory;
        return Math.round(Number(memory?.usedJSHeapSize || 0) / 1024 / 1024);
      }).catch(() => 0)
    : 0;
  return {
...
async function handle(req: any, res: any) {
  ...
  if (path === '/api/status' && req.method === 'GET') return json(res, 200, await browserStats());
  if (path === '/api/environment' && req.method === 'GET') {
    environmentReport = checkEnvironment({ includeBrowser: true });
    return json(res, 200, { report: environmentReport, message: formatEnvironmentReport(environmentReport) });
  }
  if (path === '/api/bootstrap' && req.method === 'POST') {
    const data = await body(req);
    const ready = await ensureBrowser(data.proxy);
    return json(res, ready ? 200 : 503, { success: ready, ...(await browserStats()) });
  }
  if (path === '/api/stats' && req.method === 'GET') return json(res, 200, await browserStats());
...
export function beatriceApi(): Plugin {
  return {
    ...
    configureServer(server: ViteDevServer) {
      environmentReport = checkEnvironment({ includeBrowser: true });
      server.config.logger.info(formatEnvironmentReport(environmentReport));
      ...
      if (!environmentReport.criticalMissing.length && process.env.BETTYPATCHRIGHT_SKIP_BROWSER_DOWNLOAD !== '1') {
        void ensureChromiumInstalled()
          .then(() => {
            environmentReport = checkEnvironment({ includeBrowser: true });
            server.config.logger.info('[Bettypatchright] Chromium is ready for the first browser action.');
          })
          .catch((error) => {
            patchrightState = 'error';
            patchrightError = error instanceof Error ? error.message : String(error);
            server.config.logger.error(`[Bettypatchright] Chromium preparation failed: ${patchrightError}`);
          });
      }
    },
  };
